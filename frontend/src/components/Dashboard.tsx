...
export function Dashboard({ data, loadingPlatforms, onRefreshPlatform, searchQuery }: DashboardProps) {
  ...
  const stats = useMemo(() => {
    ...
    return { total: ps.reduce((s, { items }) => s + items.length, 0), active: ps.length, sources: Object.keys(data).length, cats: Object.keys(filtered).length }
  }, [filtered, data])

  return (
    <div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        <StatCard label="总条目" value={stats.total} />
        <StatCard label="活跃平台" value={stats.active} />
        <StatCard label="数据源" value={stats.sources} />
        <StatCard label="分类" value={stats.cats} />
      </div>
      {Object.entries(filtered).map(([cat, ps], idx) => (
        <div key={cat} className={idx > 0 ? 'pt-8' : ''}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {ps.map(({ platform, items }) => <PlatformCard key={platform} platform={platform} items={items} loading={loadingPlatforms[platform]} onRefresh={onRefreshPlatform} searchQuery={searchQuery} />)}
          </div>
        </div>
      ))}
      {Object.keys(filtered).length === 0 && <div className="text-center py-20"><p className="t-body" style={{ color: 'var(--c-ash)' }}>暂无数据，请稍后刷新或检查网络连接</p></div>}
    </div>
  )
}
...
function StatCard({ label, value }: { label: string; value: number }) {
  return (
    <div className="stat-card">
      ...
      <div className="t-stat-label mt-1">{label}</div>
    </div>
  )
}
