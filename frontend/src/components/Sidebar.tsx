    ...
    <>
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-40 w-56 flex flex-col transform transition-transform lg:transform-none ${open ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}
        ...
        <div className="flex items-center gap-2.5 p-5 pb-6">
          <div className="w-7 h-7 rounded-p-md flex items-center justify-center" style={{ background: 'var(--c-primary)' }}>
            <span style={{ color: '#23251d', fontSize: '12px', fontWeight: 700 }}>H</span>
          </div>
          <h1 className="t-heading" style={{ fontSize: '16px' }}>热搜监控</h1>
        </div>
          ...
          {NAV.map(({ view, icon, label }) => {
            ...
            return (
              <button key={view} onClick={() => { onViewChange(view); setOpen(false) }}
                className="w-full flex items-center gap-2.5 px-3 py-2 rounded-p-sm transition-all"
                style={{ fontSize: '14px', fontWeight: active ? 600 : 400, color: active ? '#23251d' : 'var(--c-body)', background: active ? 'var(--c-primary)' : 'transparent' }}
